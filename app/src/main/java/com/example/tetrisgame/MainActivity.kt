...
class MainActivity : AppCompatActivity() {
    ...
    private fun setBest() {
        ...
        // Retrieve the value stored in the "high_score" key from the shared preferences
        // and assign it to the Level.best variable
        Level.best = sharedPreference.getInt("high_score", 0)
    }
    private fun resetBest(){
        val sharedPreference = getSharedPreferences("HIGH_SCORE", Context.MODE_PRIVATE)
        //  // Retrieve the shared preferences object associated with the "HIGH_SCORE" file
        if (Level.score > sharedPreference.getInt("high_score", 0)) {
            // If the current score is higher, update the shared preferences with the new high score
            var editor = sharedPreference.edit()
            editor.putInt("high_score", Level.score)
            editor.commit()
            // // Update the Level.best variable with the new high score
            Level.best = sharedPreference.getInt("high_score", 0)
        }
    }
}